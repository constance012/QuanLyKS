...
public class NguoiThuePhong extends ThuePhong implements Constants {
	...
	public NguoiThuePhong(int maKH, String tenKH, String gioiTinh, String soDT, String soCMND, String diaChi,
			String email, int soNguoi, int soNgayThue, int maPhg, double soTienThuePhong, double soTienKhachTra,
			int maHoaDon) {
		...
		bill = new ChiTietHoaDon(maKH, tenKH, diaChi, email, soDT, soCMND, soNguoi, soNgayThue, maPhg, soTienThuePhong,
				soTienKhachTra, maHoaDon);
	}
	...
	public void setSoCMND() {
		...
		while (true) {
			...
			if (soCMND.length() == 12 || soCMND.length() == 9) {
				...
				System.out.println("So CMND ko hop le, moi nhap lai:");
		}
	}
	...
	public void nhap(int maPhg, double soTienThuePhong, double soTienKhachTra, int maHoaDon) {
		try {
			...
			if (super.getSoNguoi() > 2)
				...
				soTienThuePhong *= (super.getSoNgayThue() * 0.7);
			bill = new ChiTietHoaDon(maKH, tenKH, diaChi, email, soDT, soCMND, soNguoi, soNgayThue, maPhg,
					soTienThuePhong, soTienKhachTra, maHoaDon);
